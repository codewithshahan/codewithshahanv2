...
// Define a more compatible Article interface that works with the API responses
interface Article {
  id: string;
  title: string;
  slug: string;
...
export default function CategoryPage({ params }: CategoryPageProps) {
  ...
  const [showGumroadCard, setShowGumroadCard] = useState(true);
      ...
      try {
        ...
        try {
          ...
          const formattedArticles = articles.map((article: any) => ({
            id: article.id || article._id || article.slug,
            title: article.title,
            slug: article.slug,
            brief: article.brief,
            ...
            readTime: article.readTime || article.readingTime || 3,
            readingTime: article.readingTime || article.readTime || 3,
            views: article.views,
          ...
          // Set empty articles array on error
          setArticles([]);
        }
      } catch (error) {
  ...
  const filteredArticles = React.useMemo(() => {
    ...
    return articles.filter((article) => {
      ...
      const descMatch = (article.brief || article.description || "")
        ?.toLowerCase()
        .includes(searchQuery.toLowerCase());
    ...
    <Providers>
      ...
      >
        ...
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50">
          <MacOSDock currentPath={`/categories/${slug}`} />
        </div>
